...
return function()
{
	...
	this.curPieceCode = 100; // current piece code
	this.curPlayer=0; // current player: 0 - white, 8 - black
	this.pawn2SquareMove=0; // position of pawn after two-square move, 0 after any other move
	this.moveFrom; // source position of the move
	this.moveTo; // target position of the move

	SetupChessboard();
	
	this.drawCallback = function(){console.log('please set the drawCallback')};
	...
	function compute(w,cutoff,currDepth,startCell,pawn2SquareMoveParam,scanDepth) {
		var t,
			...
			C,
			J,
			a = this.curPlayer ? -C10 : C10, // +10 for white, -10 for black
			moves;
		this.curPlayer ^= 8;
		C120++;
		...
		do {
			...
			if (o) { // if cell isn't empty
				q = o & C15 ^ this.curPlayer; // current piece code (1..6 for current player, 9..14 for opponent, 7 or 15 - outside)
				if (q < 7) {
					q--;
					...
					do {
						...
						if (!w | p == w) { // required target cell not specified or equals to current target
							...
							if ((!r & (!!q | C>1 || !!g)) || ((r+1&C15^this.curPlayer)>9 && (q | C<2))) {
								...
								t = (q | E-7) ? n : (n+=2,6^this.curPlayer);
								while (n <= t) {
									// Heuristic:
									// For attack moves:
									//   + attacked piece cost
									...
									if ((scanDepth > currDepth) || ((1<scanDepth & scanDepth==currDepth) && (L>C15 | d))) {
										...
										if (!currDepth && scanDepth == 1 && this.moveFrom == O && this.curPieceCode == n && p == this.moveTo && L >= -C10000) {
											//DrawPieces();
											this.drawCallback();
											this.moveFrom=this.moveTo;
											console.log('after drawCallback');
											C120--;
											this.pawn2SquareMove = J;
											return this.pawn2SquareMove;
										}
	...
	this.debug = function()
	{
		...
		console.log("cpc: " + this.curPieceCode);
		console.log("cp: " + this.curPlayer);
		console.log("p2sm: " + this.pawn2SquareMove);
		console.log("mf: " + this.moveFrom);
		console.log("mt: " + this.moveTo);
	}
